...
For quick agent queries without managing Query CRs directly, use the execute endpoint:

```bash
curl -X POST http://localhost:8000/v1/agents/my-agent/execute \
...
**Parameters:**
- `input` (required): The query text
- `wait` (optional, default: true): Wait for query completion
- `timeout` (optional, default: "300s"): Maximum wait time
- `sessionId` (optional): Session ID for context retention
- `memory` (optional): Memory resource name
- `parameters` (optional): Additional parameters

This endpoint is ideal for:
- Simple agent queries
- Integration with external tools (like [n8n](/integrations/n8n))
