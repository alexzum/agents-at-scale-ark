...
The `fark` CLI tool provides a convenient way to query agents and teams directly from the command line. It supports basic querying, file input, and session management.

```bash
# Trigger query named weather-query with new input
fark query weather-query "What's the weather in London?"
```

For complete CLI documentation and advanced usage examples, see the [CLI Tools Developer Guide](/developer-guide/cli-tools).
