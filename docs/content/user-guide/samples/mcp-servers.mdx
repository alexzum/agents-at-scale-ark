...
**Key Design:**
- **Shared Base**: All sessions share `/data/` via `FilesystemContext`
- **Annotation-Driven**: Workspaces configured via `set_base_directory` tool calls
- **Generic Wrapper**: Can be reused for other MCP servers (no filesystem coupling)
- **Workspace Isolation**: Achieved through explicit naming, not session IDs

This ensures flexibility and reusability while providing workspace-based isolation.
```
