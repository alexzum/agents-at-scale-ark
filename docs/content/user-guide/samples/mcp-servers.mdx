...
The Filesystem MCP Server provides persistent file operations with session-based isolation. Each session gets its own isolated workspace while supporting session persistence across pod restarts and automatic cleanup via LRU eviction.

### Features

- **Session Isolation**: Each session operates in an isolated directory (`/data/{sessionId}/`)
- **Persistent Sessions**: Sessions survive pod restarts via file-based storage
- **LRU Eviction**: Automatically evicts least recently used sessions when limit reached
- **Reconnection**: Clients can reconnect with the same session ID and access their files
...
env:
  ...
  CLEANUP_SESSION_FILES: "false"          # Delete files on session eviction

persistence:
  enabled: true
  size: 10Gi                               # Storage for session files
...
**Automatic Session Creation**: Each `ark query` command creates a new isolated session with its own workspace directory (`/data/{session-id}/`). The ARK controller manages session IDs automatically.

**Session Isolation**: Files created in one session are completely isolated from other sessions:

```bash
# First query - creates Session A with data.txt
ark query agent/filesystem "Create file data.txt with content 'Session A data'"

# Second query - creates NEW Session B, cannot see Session A's files
ark query agent/filesystem "Read data.txt"
# Result: File not found (because this is a different session)
```
...
| `mcp-filesystem-set-base-directory` | Add allowed directory |

### Advanced: Shared Directory Access

By default, each session can only access its own isolated directory. To grant access to shared directories:

```yaml
apiVersion: ark.mckinsey.com/v1alpha1
kind: Query
metadata:
  name: shared-files-query
  annotations:
          ...
          "toolCalls": [
            {
              ...
              "arguments": {"path": "/shared/team-docs"}
            }
          ]
        }
      }
spec:
  input: List files in the shared team documents
  targets:
  - type: agent
    name: filesystem
```

This grants the session access to both its isolated directory and `/shared/team-docs`.
...
**Session logs show:**
- Session creation: `[Session] Created new session <uuid> with path: <uuid>`
- Reconnection: `[Session] Reconnected session <uuid>`
- Eviction: `[Session] Evicted oldest session: <uuid>`
- Deletion: `[Session] Deleted session: <uuid>`
...
See `mcp/filesystem-mcp/LOCAL_TESTING.md` for comprehensive test scenarios including session persistence, reconnection, and LRU eviction testing.

### Architecture

The filesystem MCP server uses a wrapper + adapter architecture:
...
Each session gets an isolated `FilesystemContext` instance with its own:
- Base directory (`/data/{sessionId}/`)
- Allowed directories list (for security)
- Path validation logic

This ensures complete isolation between sessions while allowing reconnection with the same configuration.
```
