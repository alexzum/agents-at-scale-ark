...
function ResponseContent({
  ...
  content: string,
  rawJson?: unknown,
  viewMode: 'text' | 'markdown' | 'json'
}) {
  const markdownContent = useMarkdownProcessor(content)

  if (viewMode === 'json') {
    return (
      <div className="px-3 pb-3 pt-2 max-h-96 overflow-auto">
        <JsonDisplay value={rawJson ?? content} />
      </div>
    )
  }
...
interface QueryStatus {
  ...
  responses?: Array<{
    ...
    content?: string
    [k: string]: unknown // keep whole envelope for raw JSON tab
  }>
  evaluations?: Array<{
...
function QueryDetailContent() {
  ...
  // include 'json' and make it the initial default (users can switch)
  const [responseViewMode, setResponseViewMode] = useState<'text' | 'markdown' | 'json'>('json')
  ...
  const getSchemaExample = (schema: Record<string, unknown>): string | null => {
    ...
    if (schema.type === 'object' && schema.properties) {
      ...
      for (const [key, prop] of Object.entries(properties)) {
        ...
        else if (prop.default !== undefined) example[key] = prop.default
        else {
      ...
      if (Object.keys(example).length > 0) return JSON.stringify(example, null, 2)
    }
  ...
  // load query or initialize new form
  useEffect(() => {
    if (isNew) {
      setQuery({
        name: '',
    ...
    const loadQuery = async () => {
      try {
        ...
        // Keep JSON as initial view when there is any status/responses
        if ((queryData as any)?.status) {
  ...
  // load tool schema when exactly one tool selected
  useEffect(() => {
            ...
            <div className="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <table className="w-full">
                <tbody>
                  ...
                  <tr className="border-b border-gray-100 dark:border-gray-800">
                    <td className={FIELD_HEADING_STYLES}>Evaluators</td>
                    <td className="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                      {query.evaluators?.length ? `${query.evaluators.length} evaluator(s)` : "—"}
                    </td>
                  </tr>
                  <tr>
                    <td className={FIELD_HEADING_STYLES}>Eval. Selector</td>
                    <td className="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <ScrollArea className="flex-1 p-3">
            <div className="space-y-3">
              ...
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
                {query.status?.phase === 'running' ? (
                  <>
                    <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 border-b flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 text-xs">
                        <button
                          className={`px-2 py-1 rounded ${responseViewMode === 'text' ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'}`}
                          onClick={() => setResponseViewMode('text')}
                        >
                          Text
                        </button>
                        <button
                          className={`px-2 py-1 rounded ${responseViewMode === 'markdown' ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'}`}
                          onClick={() => setResponseViewMode('markdown')}
                        >
                          Markdown
                        </button>
                        <button
                          className={`px-2 py-1 rounded ${responseViewMode === 'json' ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'}`}
                    ...
                    {/* When running, still show raw JSON status if JSON tab is selected */}
                    {responseViewMode === 'json' ? (
                      <div className="px-3 py-2">
                        <JsonDisplay value={query.status ?? query} />
                      </div>
                    ) : (
                  ...
                  <Tabs defaultValue="0" className="w-full">
                    <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 border-b flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <h3 className="text-xs font-medium text-gray-600 dark:text-gray-400">Responses</h3>
                        <TabsList
                          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 h-7"
                          style={{ gridTemplateColumns: `repeat(${Math.min(query.status.responses.length, 4)}, 1fr)` }}
                        >
                  ...
                  <>
                    ...
                    {/* No responses: show raw status in JSON tab */}
                    {responseViewMode === 'json' ? (
                      <div className="px-3 py-2">
                        <JsonDisplay value={query.status ?? query} />
                      </div>
              ...
              {/* ---------- Read-only Chat UI (Text only) ---------- */}
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 border-b">
                  <h3 className="text-xs font-medium text-gray-600 dark:text-gray-400">Chat (Text)</h3>
                </div>
                <div className="p-3 space-y-3">
                  {/* User message */}
                  <div className="flex gap-2">
                    <div className="shrink-0 rounded-full w-6 h-6 bg-blue-500 text-white flex items-center justify-center text-[10px] mt-1">U</div>
                    <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg px-3 py-2 text-sm max-w-[80%]">
                      <pre className="whitespace-pre-wrap break-words font-sans">{query.input || "—"}</pre>
                    </div>
                  </div>

                  {/* Assistant messages */}
                  {query.status?.responses && query.status.responses.length > 0 ? (
                    (query.status.responses as Array<{ content?: string }>).map((r, i) => (
                      <div key={i} className="flex gap-2">
                        <div className="shrink-0 rounded-full w-6 h-6 bg-gray-700 text-white flex items-center justify-center text-[10px] mt-1">A</div>
                        <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg px-3 py-2 text-sm max-w-[80%]">
                    ...
                    <div className="text-xs text-muted-foreground">No assistant messages yet.</div>
                  )}
                </div>
              </div>
              {/* --------------------------------------------------- */}
