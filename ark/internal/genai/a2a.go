...
func handleA2AResponse(ctx context.Context, result interface{}, serverURL string, headers map[string]string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("response result is not a map or string, got type %T: %+v", result, result)
	}

	// Check if this is a TRPC A2A Go MessageProcessingResult wrapper
	if wrappedResult, hasResult := resultMap["result"]; hasResult {
		if wrappedMap, ok := wrappedResult.(map[string]interface{}); ok {
			resultMap = wrappedMap
		}
	}
...
func pollTaskStatus(ctx context.Context, taskID, serverURL string, headers map[string]string) (string, error) {
	// Prepare task status request using TRPC A2A Go standard method
...
func processTaskStatus(taskMap map[string]interface{}) (string, bool, error) {
	...
	case protocol.TaskStateCompleted:
		...
		if artifacts, ok := taskMap["artifacts"].([]interface{}); ok && len(artifacts) > 0 {
			...
			logf.Log.Info("DEBUG: Extracted from artifacts", "result", result, "error", err)
